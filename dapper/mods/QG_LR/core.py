...
#########################
try:
  from dapper.mods.QG_LR.f90.py_mod import interface_mod as fortran
except ImportError as err:
  raise Exception("Have you compiled the (Fortran) model?" + \
      "\nSee README in folder "+dirs['dapper']+"/mods/QG_LR/f90") from err

default_prms = OrderedDict([
  # These parameters may be interesting to change.
  ["dtout"        , 10.0        ], # dt for output to DAPPER.
  ["dt"           , 2.5        ], # dt used internally by Fortran. CFL = 2.0
...
sample_filename = os.path.join(dirs['samples'],'QG_samples_LR_10000.npz')
