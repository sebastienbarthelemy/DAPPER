...
from dapper.mods.QG_LR.core import model_config, shape, sample_filename, LP_setup
...
Ny = 300
jj_hr = equi_spaced_integers(129**2,Ny)
jj_hr = jj_hr-jj_hr[0]

def int2coord(n,nx):
  return [n%nx,int(np.floor(n/nx))]
...
def obs_inds(t):
  jj_hr_tmp = jj_hr + random_offset(t)
  jj_lr = []
  for i in jj_hr_tmp:
     X = int2coord(i,nxh)
     if X[0]%2==1 and X[1]%2==1:
        X[0] -= 1
        X[1] -= 1
     elif X[0]%2==1 and X[1]%2==0:
        X[0] -= 1
     elif X[0]%2==0 and X[1]%2==1:
        X[1] -= 1
     X[0] = int(X[0]/2)
     X[1] = int(X[1]/2)
     jj_lr.append(coord2int(X,nxl))
  return np.array(jj_lr)
